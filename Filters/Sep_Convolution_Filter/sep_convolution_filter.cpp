...
#pragma HLS INLINE
	...
	//Variables for no-DSP synthesis
	ap_int<16> temp1 = 0;
	ap_int<16> temp2 = 0;
	ap_int<16> temp3 = 0;
...
//Uncomment pragmas below to avoid use of DSP
// #pragma HLS RESOURCE variable=temp_v core=Mul_LUT
// #pragma HLS RESOURCE variable=temp_h core=Mul_LUT
...
// #pragma HLS RESOURCE variable=result core=Mul_LUT
	...
	horizontal_shift: for(int j = 0; j < KERNEL_WIDTH-1; j++)
	{
		...
		temp2 = convolution_buffer[j]*kernel_h[j];
		temp_h = temp_h + temp2;
	}

	temp3 = temp_v*kernel_h[KERNEL_WIDTH-1];
	temp_h = temp_h + temp3;
	convolution_buffer[KERNEL_WIDTH-1] = temp_v;

	result = temp_h / kern_sum;

	// Return of "result + kern_off" instead of just "result" it's been made as a workaround
	// to avoid use of DSPs (returning "result" will indesirably use few DSPs)
	return (result + kern_off)(PIXEL_BITS - 1,0);
}
...
#pragma HLS ARRAY_PARTITION variable=kernel_h complete dim=0
	...
	j = 0;
#else
	s_int kernel_h[KERNEL_WIDTH] = { 1, 2, 1 };
